            ...
            <div class="analysis d-f-c" style="position: relative" id="analysis_container">
                ...
                <div class="analysis-grid-container">
                    ...
                    <div class="analysis-grid ">
                        <div class="card d-f-c">
                            <h3>pending</h3>
                            <span><div class="color" style="background: orange"></div><span class="text" id="chart_category_pending">0</span></span>
                        </div>
                    </div>
                    <div class="analysis-grid ">
                        <div class="card d-f-c">
                            <h3>Rejected</h3>
                            <span><div class="color" style="background: red"></div><span class="text" id="chart_category_rejected">0</span></span>
                        </div>
                    </div>
                    <div class="analysis-grid ">
                        <div class="card d-f-c">
                            <h3>Not actioned</h3>
                            <span><div class="color" style="background: gray"></div><span class="text" id="chart_category_not_actioned">0</span></span>
                        </div>
                    </div>
                    <div class="analysis-grid ">
                        <div class="card d-f-c">
                            <h3>In Progress</h3>
                            <span><div class="color" style="background: green"></div><span class="text" id="chart_category_in_progress">0</span></span>
                        </div>
                    </div>
                    <div class="analysis-grid ">
                        <div class="card d-f-c">
                            <h3>Completed</h3>
                            <span><div class="color" style="background: navy"></div><span class="text" id="chart_category_completed">0</span></span>
                        </div>
                    </div>
                </div>
            ...
            <div class="dasboard-filter" style="padding-left: 0px">
                <div class="dashboard-filter-box">
                    ...
                    <select name="by_status" id="by_status">
                        ...
                        <option value="{{ config('constants.IN_PROGRESS') }}" @if(isset($default_status) && $default_status==config('constants.IN_PROGRESS')) {{"selected"}} @endif >In progress</option>
            ...
            <div class="pop-up-box">
                <div class="header">
                    <div class="header-box">
                        <div class="img-box">
                            <img id="sender_img" src="https://cdn.dribbble.com/users/1041205/screenshots/3636353/dribbble.jpg" alt="">
                        </div>
                ...
                <div class="pop-up-body">
                    ...
                    <div class="pop-up-row" id="grivance-img">
                        ...
                        <span class="value" >
                            <img src="https://cdn.riffre.com/filmbees/wallpapers/2015/01/27/11673/full/Avtar-11673.jpg" alt="" id="pop-up-grievance-img">
                        </span>
                    </div>
                    <div class="pop-up-row">
                        <span class="title ">
    ...
    <script>
        ...
        var dashboard = {
            ...
            setButtonByRoleAndStatus:function(role,status,id){
                ...
                if(role=={{config("constants.HOD_ROLE")}} && status=={{config("constants.APPROVED")}}){
                    return '<button class="success" style="margin-right:5px" onclick="dashboard.openActionPopUp(`in_progress`,'+id+')">add to progress</button>';
                }
                if(role=={{config("constants.HOD_ROLE")}} && status=={{config("constants.IN_PROGRESS")}}){
                    return '<button class="success" style="margin-right:5px" onclick="dashboard.openActionPopUp(`complete`,'+id+')">complete</button>';
                }
                if((role=={{config("constants.PRINCIPAL_ROLE")}} || role=={{config("constants.PRINCIPAL_ROLE")}}) && status=={{config("constants.PENDING")}}){
                    return '<button class="success"  style="margin-right:5px" onclick="dashboard.openActionPopUp(`approve`,'+id+')">approve</button><button class="danger" onclick="dashboard.openActionPopUp(`rejected`,'+id+')">declaine</button>';
                }
                if((role=={{config("constants.PRINCIPAL_ROLE")}} || role=={{config("constants.PRINCIPAL_ROLE")}}) && status=={{config("constants.APPROVED")}}){
                    return '<button class="success" style="margin-right:5px" onclick="dashboard.openActionPopUp(`in_progress`,'+id+')">add to progress</button>';
                }
            ...
            setStatus:function (status){
                switch(parseInt(status)){
                    case {{config("constants.PENDING")}} : return '<span class="status orange" >pending </span>';
                        break;
                    case {{config("constants.REJECTED")}} : return '<span class="status danger" >Rejected </span>';
                        break;
                    case {{config("constants.APPROVED")}} : return '<span class="status success" >approved </span>';
                        break;
                    case {{config("constants.SEEN")}} : return '<span class="status gray" >not actioned</span>';
                        break;
                    case {{config("constants.IN_PROGRESS")}} : return '<span class="status success" >In progress </span>';
                        break;
            ...
            setCount:function(data){
                ...
                for(let temp of data){
                    ...
                    switch(parseInt(temp.status)){
                        ...
                        break;
                        case {{config('constants.REJECTED')}}: status_count['rejected'] = temp.total;
                        break;
                        case {{config('constants.APPROVED')}}: status_count['not_actioned'] = temp.total;
                        break;
                        case {{config('constants.IN_PROGRESS')}}: status_count['in_progress'] = temp.total;
                        break;
